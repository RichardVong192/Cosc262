...
class KdTree:
    ...
    def nearest_point(self, p):
        min_dist_point = min(self.what_distance_between_point_in_node(p), key=lambda q : (q - p).len(sq))
        q = min_dis_point
        d = (q - p).lensq()
        query_rectangle = (Vec(p.x - d, p.y - d), Vec(p.x + d, p.y +d))
        matches = self.points_in_range(query_rectangle)
        return min(matches, key=lambda s: (s-p).lensq())
    
    def what_distance_between_point_in_node(self, p):
        """returns the node that a point is contained in"""
        matches = []
        if self.is_leaf:
            matches = self.points
        else:
